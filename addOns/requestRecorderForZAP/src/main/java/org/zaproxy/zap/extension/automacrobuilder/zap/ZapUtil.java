...
public class ZapUtil {
     ...
     *
     * @param mbui
     * @param isOriginalRequest - if true then this function returns original PRequest<br>
     *     instead of current viewed request.
     * @return
     */
    public static PRequest getPRequestFromMacroRequest(
            MacroBuilderUI mbui, boolean isOriginalRequest) {
        ...
        if (pos > -1 && pmt != null) {
            ...
            if (!isOriginalRequest) {
                ...
                if (doc != null) {
                    PRequest newrequest =
                            doc.reBuildPRequestFromDocTextAndChunksWithEncodeCustomTag();
    ...
    public static PRequestResponse getPRequestResponse(HttpMessage htmess, Encode sequenceEncode) {
        ...
        // must use getContent method. it can apply properly decoding which is based on
        // Content-Encoding
        ...
        // must use getContent method which can get properly decoded value which is based on
        // Content-Encoding
    ...
    public static PRequest getPRequest(HttpMessage htmess, Encode lastResponseEncode) {
        ...
        ParmGenBinUtil requestbin = new ParmGenBinUtil(requestheader.toString().getBytes());
        // requestbin.concat(requestbody.getBytes());
        // must use getContent method which apply properly decoding based on
        // Content-Encoding
        requestbin.concat(requestbody.getContent());
        String host = requestheader.getHostName();
        int port = requestheader.getHostPort();
    ...
    public static Extension getExtensionByName(String extensionName) {
        return Control.getSingleton().getExtensionLoader().getExtension(extensionName);
    }
    ...
    public static boolean callCustomActiveScanMethod(
            Object object,
            String packageName,
            String methodName,
            Class<?>[] clazzArray,
        ...
        Object[] exampleargument = new Object[] {123, "a"};

        Class<?>[] exampleClazz = {Integer.class, String.class};
    ...
    public static <T> T callCustomActiveScanMethodReturner(
            Class<T> returnClass,
            Object object,
            String packageName,
            String methodName,
                    ...
                    if (clazzArray == null
                        ...
                        returnValue =
                                CastUtils.castToType(
                                        returnClass, method.invoke(object, objectArray));
                        LOGGER4J.debug("invoked.cls:" + cls.getName() + " method:" + methodName);
                        called = true;
                    }
                } else {
    ...
    public static String urlDecodePartOfCustomEncodedText(String text) {
        ...
        if (!positions.isEmpty()) {
            ...
            for (StartEndPosition position : positions) {
                ...
                decodedTextBuff.append(
                        ZapUtil.decodeURL(
                                text.substring(position.start, position.end), Encode.UTF_8));

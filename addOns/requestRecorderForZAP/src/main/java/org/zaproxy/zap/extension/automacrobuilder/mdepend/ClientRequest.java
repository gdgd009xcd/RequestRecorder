...
public class ClientRequest implements InterfaceClientRequest {
            ...
            try {
                ...
                LOGGER4J.debug(
                        "STEP["
                                + pmt.getStepNo()
                                + "] send URL:"
                                + htmess.getRequestHeader().getURI());
                ...
                // must use getContent method which get body bytes with applying properly decoding
                // which is based on Content-Encoding
    ...
    public HttpMessage startZapCurrentRequest(ParmGenMacroTrace pmt, HttpMessage currentmessage) {
        ...
        if (retval != null) {
            ...
            LOGGER4J.debug(
                    "ZapUtil.getHttpMessage URL["
                            + newmessage.getRequestHeader().getURI().toString()
                            + "]");
    ...
    public void postZapCurrentResponse(ParmGenMacroTrace pmt, HttpMessage currentmessage) {
        LOGGER4J.info(
                "STEP["
                        + pmt.getStepNo()
                        + "] current send URL:"
                        + currentmessage.getRequestHeader().getURI());
    ...
    public void updateCurrentResponseWithFinalResponse(
        ...
        LOGGER4J.debug(
                "isURIOfRequestIsModified:" + (pmt.isURIOfRequestIsModified() ? "true" : "false"));
        if (pmt.isCBFinalResponse() && !pmt.isURIOfRequestIsModified()) {
            ...
            if (finalresponse != null) {
                try {
                    ...
                    // set body bytes with applying properly encoding which is based on
                    // Content-Encoding
